...
- **Domain is `triad_ams`** (snake_case, matches directory). The matching HA device class is `media_player`. The integration owner is `@krizzo` (`codeowners` in manifest); originally authored by `@bharat`, see the README credits.
