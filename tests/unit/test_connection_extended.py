...
class TestPaddedResponses:
    ...
    async def test_consecutive_commands_skip_response_padding(self) -> None:
        ...
        adaptive drain must discard the padding of response N so response
        N+1 is read cleanly (observed live: without this, every command
        after the first sees a stale/empty response). See issue #164.
        """
        conn, _ = _wired_connection(
            "Get Power status : Working", "Fw version : V1.0054"
        )
        padding = [b"\x00"] * 123

        def readexactly_side_effect(*_args: object, **_kwargs: object) -> bytes:
            if padding:
                return padding.pop(0)
            raise TimeoutError

        conn._reader.readexactly = create_async_mock_method(
